...
class MapEnvironment(object):
    ...
    def compute_distance(self, start_config, end_config):
        ...
        # Euclidean distance
        diff_x = abs(start_config[0,0] - end_config[0,0])
        diff_y = abs(start_config[1,0] - end_config[1,0])
    ...
    def state_validity_checker(self, config):
        ...
        """
        # TODO: YOUR IMPLEMENTATION HERE
        for index in range(config.shape[1]):
            x = int(round(config[0, index]))
            y = int(round(config[1, index]))
    ...
    def h(self, config):
        ...
        # Distance between a certain point and the goal
